        ...
        <div className={styles.layout}>
          ...
          <div className={styles.itemsList}>
            <div className={styles.listHeader}>
              ...
              {showClearConfirm ? (
                <div style={{ display: 'flex', gap: '8px', alignItems: 'center', animation: 'fadeUp 0.2s ease' }}>
                  <span style={{ fontSize: '0.8rem', color: 'var(--tm)' }}>Are you sure?</span>
                  <button
                    className={styles.clearBtn}
                    style={{ borderColor: '#c01818', color: '#c01818', background: 'rgba(192, 24, 24, 0.05)' }}
            ...
            {items.map(item => {
              ...
              return (
                <div key={item.id} className="cart-item">
                  ...
                  <button className={styles.removeBtn} onClick={() => removeItem(item.id)} aria-label={`Remove ${item.name} from cart`} title="Remove">✕</button>
                </div>
              );
            })}
          </div>
