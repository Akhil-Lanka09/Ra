          ...
          <div className={styles.itemsList}>
            <div className={styles.listHeader}>
              ...
              <button
                ...
                aria-label={confirmClear ? "Confirm clear all items" : "Clear all items"}
              >
              ...
              return (
                <div key={item.id} className="cart-item">
                  ...
                  <div className={styles.qtyControl}>
                    <button className={styles.qtyBtn} onClick={() => updateQty(item.id, item.qty - 1)} aria-label="Decrease quantity">−</button>
                    <span className={styles.qtyNum}>{item.qty}</span>
                    <button className={styles.qtyBtn} onClick={() => updateQty(item.id, item.qty + 1)} aria-label="Increase quantity">+</button>
                  </div>
                  <button className={styles.removeBtn} onClick={() => removeItem(item.id)} title="Remove" aria-label="Remove item">✕</button>
                </div>
              );
            })}
          </div>
